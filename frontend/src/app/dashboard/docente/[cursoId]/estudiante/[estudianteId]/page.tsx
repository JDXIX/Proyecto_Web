...
export default function DetalleEstudianteDocentePage({
  params,
}: {
  params: { cursoId: string; estudianteId: string };
}) {
  ...
  const { cursoId, estudianteId } = params;
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:8000/api/historial-estudiantes/?estudiante=${estudianteId}&curso=${cursoId}`,
        {
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-xl shadow p-8">
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            ...
            {historial.length === 0 && (
              <div className="text-gray-500 mt-4">No hay historial registrado para este estudiante.</div>
            )}
          </div>
        )}
      </div>
